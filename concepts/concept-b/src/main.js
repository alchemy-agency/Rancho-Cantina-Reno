...
if (!reduce) {
  gsap.timeline({ defaults: { ease: 'power3.out' } })
    ...
    .from('.hero__sound, .hero__corner, .hdr', { opacity: 0, duration: 1 }, 1.1)
}
...
const openModal = () => {
  ...
  if (date && !date.value) { const d = new Date(); d.setDate(d.getDate() + 1); date.value = d.toISOString().slice(0, 10); date.min = new Date().toISOString().slice(0, 10) }
...
const closeModal = () => { modal.hidden = true; if (lenis) lenis.start(); document.documentElement.style.overflow = ''; lastFocus?.focus?.() }
